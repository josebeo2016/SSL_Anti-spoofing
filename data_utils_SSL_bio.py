...
class Dataset_ASVspoof2019_train(Dataset):
    def __init__(self,args,list_IDs, labels, base_dir,algo, use_bio=False):
    ...
    def __getitem__(self, index):
        ...
        if self.use_bio:
            bio_inp, bio_length = self.get_Bio(utt_id, X_pad, fs)
        # bio_inp, bio_length = self.get_Bio(utt_id, X_pad, fs) 
        x_inp= Tensor(X_pad)
        target = self.labels[utt_id]
        if self.use_bio:
            return x_inp, bio_inp, bio_length, target
        else:
            return x_inp, None, None, target
...
class Dataset_ASVspoof2021_eval(Dataset):
    def __init__(self, list_IDs, base_dir, use_bio=True):
        self.list_IDs = list_IDs
        self.base_dir = base_dir
        self.use_bio = use_bio
        self.cut=64600 # take ~4 sec audio (64600 samples)
    ...
    def __getitem__(self, index):
        ...
        x_inp = Tensor(X_pad)
        if self.use_bio:
            return x_inp, bio_inp, bio_length,utt_id
        else:
            return x_inp,None,None,utt_id
...
class Dataset_ASVspoof2019_eval(Dataset):
    ...
    def __getitem__(self, index):
        self.cut = 64600  # take ~4 sec audio (64600 samples)
        utt_id = self.list_IDs[index].split()[1]
