...
import yaml
from data_utils_acmccs import genSpoof_list, genSpoof_list_custom, Dataset_for, Dataset_for_eval
from model import Conformer as Model
...
if __name__ == '__main__':
    ...
    '''
    parser.add_argument('--protocol', type=str, default='protocol.txt', help='protocol file name in the database path')
    ...
    parser.add_argument('--eval', action='store_true', default=False,
                        help='eval mode')
    parser.add_argument('--weight_avg', type=str, default=None,
                        help='Average the last at most 5 model weights, then inference')
    ...
    if (args.model_path) and (args.weight_avg is None):
        try:
    ...
    if args.weight_avg:
        # load the last 5 model weights and average them
        model_files = os.listdir(args.weight_avg)
        model_files = [f for f in model_files if f.endswith('.pth')]
        # the checkpoint files are named as epoch_{epoch}.pth
        model_files = sorted(model_files, key=lambda x: int(x.split('_')[1].split('.')[0]))
        model_files = model_files[-5:]
        print('Averaging the following model weights of {} models:'.format(len(model_files)))
        state_dicts = []
        average_state_dict = {}
        for f in model_files:
            state_dicts.append(torch.load(os.path.join(args.weight_avg, f)))
        for key in state_dicts[0].keys():
            average_state_dict[key] = sum([state_dict[key] for state_dict in state_dicts])/len(state_dicts)
        try:
            ...
            model = model.to(device)
            model = nn.DataParallel(model)
        print('Model loaded : {}'.format(args.weight_avg))
    ...
    if args.eval:
        if args.custom:
            file_eval = genSpoof_list_custom(dir_meta = os.path.join(args.database_path,args.protocol))
        else:
            file_eval = genSpoof_list( dir_meta =  os.path.join(args.database_path,args.protocol),is_train=False,is_eval=True)
        print('no. of eval trials',len(file_eval))
        eval_set=Dataset_for_eval(list_IDs = file_eval,base_dir=args.database_path+"/")
        if (args.predict):
    ...
    d_label_trn,file_train = genSpoof_list( dir_meta =  os.path.join(args.database_path,args.protocol),is_train=True,is_eval=False,is_dev=False)
    ...
    d_label_dev,file_dev = genSpoof_list(dir_meta = os.path.join(args.database_path,args.protocol),is_train=False,is_eval=False, is_dev=True)
